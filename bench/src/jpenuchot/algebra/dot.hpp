...
#define BM_DOT_PARRAY_DUMB(size) \
		static void BM_dot_parray_dumb_##size(benchmark::State& state) { \
			...
			std::iota(p.data.begin(), p.data.end(), 0.f); \
...
#define BM_DOT_PARRAY_SIMD_UNSAFE(size) \
	static void BM_dot_parray_simd_unsafe_##size(benchmark::State& state) { \
		constexpr auto sz = std::size_t((size)); \
		jp::parray<btype, sz> p; \
		std::iota(p.data.begin(), p.data.end(), 0.f); \
		while(state.KeepRunning()){ \
	...
	BENCHMARK(BM_dot_parray_simd_unsafe_##size);
